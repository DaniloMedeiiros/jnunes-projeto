...
import useTrilhas from '../../hooks/useTrilhas'; // Importa o hook
import './style.css';
...
export default function PaginaTrilhaCompleta() {
  ...
  const trilhasEmAndamento = USE_MOCKED_DATA
    ? mockTrilhasEmAndamento
    : trilhas.filter((t) => t.progresso > 0 && t.progresso < 100); // Filtra trilhas em andamento
  const trilhasDisponiveis = USE_MOCKED_DATA
    ? mockTrilhasDisponiveis
    : trilhas.filter((t) => t.progresso === 0); // Filtra trilhas disponíveis
  ...
  const navegarCarrossel = (direcao, tipoTrilha) => {
    if (tipoTrilha === 'andamento') {
      ...
      if (carrosselAndamentoRef.current) {
        const cardWidth =
          carrosselAndamentoRef.current.children[0]?.offsetWidth || 300;
      ...
      if (carrosselDisponiveisRef.current) {
        const cardWidth =
          carrosselDisponiveisRef.current.children[0]?.offsetWidth || 300;
  ...
  const handleCardClick = (trilhaId, tipoTrilha) => {
    navigate(`/trilha/${trilhaId}`, {
      state: {
        tipo: tipoTrilha,
        progresso:
          trilhasEmAndamento.find((t) => t.id === trilhaId)?.progresso || 0
      }
    });
  };
  ...
  return (
    <div className="trilha-container">
      ...
      {/* Menu Lateral */}
      <div
        className={`trilha-menu-lateral ${
          menuAberto ? '' : 'trilha-menu-fechado'
        }`}
      >
        <div className="trilha-logo-container">
          <img
            src="src/imagens/logo-aberta.png"
            className={`trilha-logo trilha-logo-aberta ${
              menuAberto ? 'trilha-visivel' : 'trilha-escondido'
            }`}
            alt="Logo"
          />
          <img
            src="src/imagens/logo-fechada.png"
            className={`trilha-logo trilha-logo-fechada ${
              !menuAberto ? 'trilha-visivel' : 'trilha-escondido'
            }`}
            alt="Logo"
          />
        ...
        {caixaAberta.notificacao && (
          ...
          >
            ...
            <ul className="trilha-mensagens">
              <li>
                <span className="trilha-ponto-vermelho">•</span> Nova mensagem
                de sistema
              </li>
              <li>
                <span className="trilha-ponto-vermelho">•</span> Atualização de
                perfil disponível
              </li>
              <li>
                <span className="trilha-ponto-vermelho">•</span> Novo evento
                agendado
              </li>
            </ul>
            <a href="#" className="trilha-link-limpar">
              Limpar Mensagens
            </a>
          </div>
        )}
      ...
      <div className="trilha-conteudo-principal">
        ...
        <div className="trilha-secao-carrossel">
          ...
          >
            {isLoading ? (
              <p>Carregando trilhas em andamento...</p>
            ) : error ? (
              <p>Erro ao carregar trilhas em andamento: {error.message}</p>
            ) : (
                ...
                >
                  ...
                  <div className="trilha-secao-status">
                    ...
                    {trilha.concluido ? (
                      ...
                      <span className="trilha-secao-andamento">
                        Em andamento
                      </span>
                    )}
                  </div>
        ...
        <div className="trilha-secao-carrossel">
          ...
          >
            {isLoading ? (
              <p>Carregando trilhas disponíveis...</p>
            ) : error ? (
              <p>Erro ao carregar trilhas disponíveis: {error.message}</p>
            ) : (
              trilhasDisponiveis.map((trilha) => (
                <div
                  key={`disponivel-${trilha.id}`}
                  className="trilha-secao-card"
                >
                  <div className="trilha-secao-imagem" />
                  <h3 className="trilha-secao-nome">{trilha.titulo}</h3>
      ...
      {/* Modais */}
      {modalVisivel.dados && (
        <div className="trilha-modal" onClick={() => toggleModal('dados')}>
          <div
            ref={modalDadosRef}
      ...
      {modalVisivel.senha && (
        <div className="trilha-modal" onClick={() => toggleModal('senha')}>
          <div
            ref={modalSenhaRef}
            className="trilha-modal-conteudo"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="trilha-dados-usuario">
              ...
              <button className="trilha-btn-alterar-senha">
                Alterar Senha
              </button>
            </div>
          </div>
